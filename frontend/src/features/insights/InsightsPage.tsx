...
function InfoPanel({ title, content }: { title: string; content: string }) {
  ...
  return (
    <div className="mt-4">
      <button onClick={() => setOpen(!open)} className="flex items-center text-sm text-[#6B7280] hover:text-[#242824] transition-colors">
        <Info className="w-4 h-4 mr-1.5" />
        {title}
      </button>
      {open && (
        <div className="mt-2 p-3 bg-gray-50 border border-[#E5E2DB] rounded-md text-sm text-[#6B7280]">
          {content}
        </div>
      )}
    </div>
...
export function InsightsPage() {
  ...
  if (topicsError || diffError || activityError || mistakesError) return <p role="alert" className="text-red-600">{(topicsError || diffError || activityError || mistakesError)?.message}</p>;
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        {/* Topic Breakdown */}
        <div className="bg-white border border-[#E5E2DB] rounded-lg p-6 shadow-sm flex flex-col">
          <h2 className="text-lg font-semibold text-[#242824] mb-4">Topic Breakdown</h2>
          <div className="h-80 w-full flex-1">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={topicsChartData} layout="vertical" margin={{ top: 5, right: 30, left: 40, bottom: 5 }}>
                ...
                <Legend />
                <Bar dataKey="Attempted" stackId="a" fill="#9CA3AF" name="Attempted (Unsolved)" />
                <Bar dataKey="Solved" stackId="a" fill="#35634E" name="Solved" />
              </BarChart>
            </ResponsiveContainer>
          </div>
          <InfoPanel 
        ...
        {/* Difficulty Bands */}
        <div className="bg-white border border-[#E5E2DB] rounded-lg p-6 shadow-sm flex flex-col">
          <h2 className="text-lg font-semibold text-[#242824] mb-4">Difficulty Bands</h2>
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-[#E5E2DB]">
                {(diffData?.bands || []).map((band, i) => (
                  <tr key={i} className="hover:bg-[#F0EFEB] transition-colors">
                    <td className="px-4 py-2 font-medium text-[#242824]">{band.label}</td>
                    <td className="px-4 py-2 text-right font-mono text-[#6B7280]">{band.attempted}</td>
                    <td className="px-4 py-2 text-right font-mono text-[#6B7280]">{band.solved}</td>
                    <td className="px-4 py-2 text-right font-mono font-medium">
                      {band.insufficientData ? (
                        <span className="text-[#9CA3AF] text-xs">Not enough data</span>
                      ) : (
                        <span className={band.solveRate && band.solveRate >= 0.8 ? 'text-[#35634E]' : band.solveRate && band.solveRate < 0.5 ? 'text-red-600' : 'text-[#242824]'}>
        ...
        {/* Weekly Activity */}
        <div className="bg-white border border-[#E5E2DB] rounded-lg p-6 shadow-sm flex flex-col">
          <h2 className="text-lg font-semibold text-[#242824] mb-4">Weekly Activity</h2>
          <div className="h-80 w-full flex-1">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={activityChartData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                ...
                <Legend />
                <Bar dataKey="Contest" stackId="a" fill="#35634E" />
                <Bar dataKey="Virtual" stackId="a" fill="#D97706" />
                <Bar dataKey="Practice" stackId="a" fill="#9CA3AF" />
              </BarChart>
            </ResponsiveContainer>
          </div>
          <InfoPanel 
        ...
        {/* Mistake Patterns */}
        <div className="bg-white border border-[#E5E2DB] rounded-lg p-6 shadow-sm flex flex-col">
          <h2 className="text-lg font-semibold text-[#242824] mb-4">Mistake Patterns</h2>
          <div className="flex-1 space-y-4 overflow-y-auto pr-2">
            {!mistakesData?.mistakes || mistakesData.mistakes.length === 0 ? (
              <div className="text-sm text-[#6B7280] text-center py-8">No mistake data logged yet. Add notes to your solved problems.</div>
            ) : (
                ...
                return (
                  <div key={i} className="flex flex-col">
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-sm font-medium text-[#242824]">{m.category.replace(/_/g, ' ')}</span>
                      <span className="text-sm font-mono text-[#6B7280]">{m.count} ({Math.round(percentage)}%)</span>
                    </div>
